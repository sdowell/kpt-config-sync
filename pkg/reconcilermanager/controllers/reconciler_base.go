...
// and upsert a new ConfigMap in the RSync's namespace.
func (r *reconcilerBase) mergeNotificationConfigs(ctx context.Context, rsKind, rsNamespace, rsName string, config *v1beta1.NotificationConfig) (string, error) {
	...
	mergedCM.Namespace = rsNamespace
	if rsKind == kinds.RootSyncV1Beta1().Kind {
		mergedCM.Name = ReconcilerResourceName(core.RootReconcilerName(rsName), cm.Name)

	} else {
		mergedCM.Name = ReconcilerResourceName(core.NsReconcilerName(rsNamespace, rsName), cm.Name)
	}
	op, err := controllerruntime.CreateOrUpdate(ctx, r.client, mergedCM, func() error {
		mergedCM.Data = cm.Data
		return nil
	})
	if err != nil {
		return "", fmt.Errorf("failed to upsert the merged notification ConfigMap %s in the %s namespace: %w", mergedCM.Name, mergedCM.Namespace, err)
	}
	if op != controllerutil.OperationResultNone {
		r.log.Info("Merge and upsert notification configs successful",
			logFieldObject, client.ObjectKeyFromObject(mergedCM).String(),
			logFieldKind, "ConfigMap",
			logFieldOperation, op)
	}
	return mergedCM.Name, nil
}
