...
func (r *RootSyncReconciler) Reconcile(ctx context.Context, req controllerruntime.Request) (controllerruntime.Result, error) {
	...
	log.Info("reached populateContainerEnvs")
	containerEnvs := r.populateContainerEnvs(ctx, rs, notificationEnabled, reconcilerRef.Name, rs.Spec.NotificationConfig)
...
func (r *RootSyncReconciler) populateContainerEnvs(ctx context.Context, rs *v1beta1.RootSync, notificationEnabled bool, reconcilerName string, notificationConfig *v1beta1.NotificationConfig) map[string][]corev1.EnvVar {
	...
	if notificationEnabled {
		result[reconcilermanager.Notification] = notificationEnvs(r.syncKind, rs.Namespace, rs.Name, notificationConfig)
	}
	switch v1beta1.SourceType(rs.Spec.SourceType) {
	case v1beta1.GitSource:
